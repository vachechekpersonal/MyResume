...
- Timeline: newest first, vertical line, first role expanded by default, others collapsed. "Expand all" / "Collapse all" send a versioned `ExpansionCommand` down as a parameter; entries apply each command once, so manual toggles between commands survive re-renders. Career break shown as a quiet marker, not a card. Each entry shows period, computed duration, role, company, location, highlights, and skill tags (selected tags highlighted).
- Section nav: sticky links to each section. Before Blazor starts they are plain fragment links; afterwards `js/sections.js` intercepts clicks (smooth scroll, `history.replaceState` so `?skills=` survives) and marks the section in view with `aria-current` via IntersectionObserver.
